...
We got this file by using the bedops program, it is a tool used to find relationships between two or more genomic datasets. In this case we used the --intersect function, which allows us to out put a .bed file that contains the regions that are common in both input files, in other words it gives the intersection of genomic space of two .bed files. This input .bed files must be sorted, which we all ready did in previous steps.
```{eval =F}
module load bedops/2.4.20
bedops --intersect Enhancers_data_paper_38.bed Promoters_data_paper_38.bed > Enh_Pro_isec38.bed
```
Because both input files are sorted, the output file will be sorted too, but in case it wasn't sorted we could run the following (this function was previously explained):
```{eval =F}
...
This gave us a .bed file with the Genomic coordinates, a .fasta file with the fetched sequences, and a .txt file with the information of the submitted job.
...
To do this we used the CORE Jaspar collection and filtered it to down to Homo sapiens latest version profiles that where optained with Chip-seq. 233 profiles where found as shown in the following link:  https://jaspar.genereg.net/search?q=&collection=CORE&tax_group=all&tax_id=9606&type=ChIP-seq&class=all&family=all&version=latest
...
For the control, we downloaded permuted versions of the 233 profiles found in Jaspar so that we have better chances of the results from the pattern matching not being by chance. For the permute settings we used type: 'within each matrix' (so that we are not biased towards the nucleotide abundance of certain TFBS regions), and  format: MEME. We download the permuted matrices to the cluster.
```{eval=F}
...
##Pattern Matching
Form the RSAT documentation we decided to use the 'matrix-scan' program for pattern matching as it allows multiple sequences and PSSM as input, as well as pre-calibrated markov models and customized background models. In this case we used the permuted matrices as background model as this plays with the nucleotide frequency in a certain region and will be a good indicator for the matrix-scan to statistically test how likely it is for a region with this nucleotide frequencies to match the TFBS by chance or because it could be a real TFBS.
...
-matrix_format meme         indicates the matrices file is in meme format
-i <sequences.fasta>        indicates the name of the file with the sequences it will use to scan the matrices with
-bgfile <backgroung_model.txt> indicates the name of the file with the background model
-bg_format meme             indicates the background model file is in meme format
-o <outout_file>            indicates the name of the output file it will write

Note: This function might take several days to run

The output file is tab-delimited with one row per match, it contains the following information:

-seq_id: Name of the sequence, in this case it has a hg38_chr<n>_<start_pos>_<end_pos>_+ format
-ft_type: it takes values of 'limit' or 'site', with site indicating the match is at the TFBS.
-ft_name: Name of the PWM corresponding to this hit
-strand: Indicates in which strand was the hit, 'D' is for direct strand and 'R' is for reverse strand.
-start: Position of the first hit relative to the fetched seq, with the first nucleotide being -1.
-end: Position of the last hit relative to the fetched seq, with the first nucleotide being -1.
-sequence: Nucleotide sequence of the potential binding site.
...
S : Sequence segment
M : PSSM 
B : Background model matrix
Note: the program uses natural logarithms

##Finding significant TFBS
To retrieve the most significant TFBS matched by our fetched sequences (that represent the genomic regions where H3K4me3 binds in both promoters and enhancers); we first look at the weight scores given by the matrix-scan program and the length of the matched sequences.

Here we are looking for the biggest weight scores. Because of how the weight is calculated, then the bigger the weight scores, the more significant the probability of the matched sequences is to be a real TFBS. We also look for fairly long sequences, as short matching sequences will easily fit into random sequences just because they are short. 

### Statistical Test
To know what our weight score threshold should be, we performed a Bonferroni statistical test, which is used to reduce false positives when dealing with multiple comparatives, in this way we aim to reduce the significance of false positives in our results:
...
Because of how the weight is calculated, then the bigger the weight scores, the more significant the probability of the matched sequences is to be a real TFBS. According to the Bonferroni test we performed, a weight score threshold of 22 should be enoght to rule out enough false positives from the matrix-scan output. We also look for fairly long sequences, as short matching sequences will easily fit into random sequences just because they are short. 

To fetch long sequences (more than 20 nucleotides) and high weight scores (grater than 22):
Note: the length and weight scores are arbitrary.
```{eval=F}
awk '{if(($6 - $5)> 20 && $8>22) print ($3)}' matrix_scan2 | uniq > ID_mat.txt
...
16
```
With this parameters 16 significant TFBS where found for our fetched sequences.

To fetch the name of the TFBS for each matrix match that passed our threshold:
```{r, eval=F}
...
### TFBS Function
Looking at JASPAR's database we can have an idea of the function of this TF:
REST -> C2H2 zinc finger factors
ESR1 -> Nuclear receptors with C4 zinc fingers
TP63 -> p53 domain factors
IRF1 -> Tryptophan cluster factors
ZNF140 -> C2H2 zinc finger factors	
ZNF382 -> C2H2 zinc finger factors
ZNF675 -> C2H2 zinc finger factors
ZNF8 -> C2H2 zinc finger factors
NFIB -> SMAD/NF-1 DNA-binding domain factors
ZFP42 -> C2H2 zinc finger factors
ZNF16 -> C2H2 zinc finger factors
SPI1 -> Tryptophan cluster factors
CTCF -> C2H2 zinc finger factors
CTCF -> C2H2 zinc finger factors
ZNF320 -> C2H2 zinc finger factors
ZNF324 -> C2H2 zinc finger factors


## Conclusion
We found 16 significantly enriched TFBS in Macrophage's promoters and enhancers. The proteins associates with this TFBS are 
...
Biola M. Javierre, Oliver S. Burren, Steven P. Wilder, ..., Chris Wallace, Mikhail Spivakov, Peter Fraser (2016). Lineage-Specific Genome Architecture Links Enhancers and Non-coding Disease Variants to Target Gene Promoters. Cell 167, 1369-1384.

Thomas-Chollier, M., Sand, O., Turatsinze, J. V., Janky, R., Defrance, M., Vervisch, E., Brohée, S., & van Helden, J. (2008). RSAT: regulatory sequence analysis tools. Nucleic acids research, 36(Web Server issue), W119–W127. https://doi.org/10.1093/nar/gkn304

Wasserman, W., Sandelin, A (2004). Applied bioinformatics for the identification of regulatory elements. Nat Rev Genet 5, 276–287. https://doi.org/10.1038/nrg1315

Zach. (2021) The Bonferroni Correction: Definition and Example. Statology Org. https://www.statology.org/bonferroni-correction/#:~:text=For%20example%2C%20if%20we%20perform,01667.&text=Thus%2C%20we%20should%20only%20reject,the%20test%20is%20less%20than%20.

matrix-scan: http://rsat.sb-roscoff.fr/help.matrix-scan.html

bedops 2.4.20 : https://bedops.readthedocs.io/en/latest/content/reference/set-operations/bedops.html
