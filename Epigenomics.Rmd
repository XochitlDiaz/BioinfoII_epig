...
-bg_format meme             indicates the backgound model file is in meme format
...
To fetch long sequences (30 nucleotides or more) and high weight scores (grater then 80):
Note: the length and weight scores are arbitrary.
```{eval=F}
awk '{if(($6 - $5)> 29 && $8>80) print ($3)}' matrix_scan2 > filtered_mat_scan2

#to see how many significant TFBS where found:
wc -l filtered_mat_scan2
#output
773772
```
With this parameters 773772 matches where found for our fetched sequences.

To fetch the name of the promoter for each matrix match that passed our threshold:
```{r, eval=F}
# Other way
# Setting FWER
a = 0.05
...
# Commands
awk '{if(($6 - $5)> 19 && $8>22) print ($3)}' matrix_scan2 | sort | uniq > ID_2.txt
grep "$(<ID_2.txt)" JASPAR_combined_matrices_meme.txt



awk '{
  for (i=1;1<=NF;i++) /filtered_mat_scan2[i]/
}
END{
  
}'
grep 'matrix_id' JASPAR_matrices_meme.txt
...
#python 3.8
#library to use grep with python
import re

#opening data in env
with open(filtered_mat_scan2, "r") as file1:
  mat= file1.readlines()

file2= open("JASPAR_matrices_meme.txt")
JPR= file2.readlines()
file2.close()
#opening output file
outF = open("TFBS.txt", "w")


#search pattern
for matid in mat:
  x= re.search('"%s".[\w-]' % (matid), JPR)
  s= x.split('.')
  x2= s[1]
  print >>outF, x2
...
-bedgraphtobigwig
>>>>>>> 29fafb7632e8aa42dfa1ca120461d77268fef60a
